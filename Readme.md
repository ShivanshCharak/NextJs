...
## Route group Layout
- To organize your projecy in a manner that doesnt affect the URL
- To selective apply a layout to certain segements while leaving the others unchanged
## Routing metadata
- Ensuring proper search enginer optimization seo is crusial for increasinf visisbility and attracting users
- Nextjs introducts the metadata Api which allows you to define metadata for each page
- Metadata ensure accurate and relevant information is deplayed when your pages are shared and indexed 
## Configuring metadata
- Export a static metadata object
- Exporta dynamic generateMetadata function
### Metadata rules
- Both layout.tsx and page.tsx can export metadata. if defined in a layout it applies to all pages in that layout but if defined in a page, it applied inly to that page

- Metadata is read in order, from the root level down to the final page level
- When there is metadata in multiple places for the same route they get combination but page metadata will replace layout metadata if they have the same property
...
// can define it as async function as well
...
```
### Title metadata
If the page.tsx does contain title for the metadata it will be replaced by the %s and default will show the default if there is no title in the metadata
```
export const metadata:Metadata = {
  title: {
    default:"Nextjs Tutorial - Code Envolution",
...
function page() {
  return (
    <div>
      ...
      <Link href="/products" replace>Product</Link>
      {/* the normal link can take us to the blog page and we click on back from the windows it will go back  but if we use replace in the link instead if going back it will go to the root directory*/}
    </div>
  )
}
...
## Active link
### usePathname()
- usePathname returns the current pathname as a string, allowing you to use this value in your components. This hook can be especially handy for highlighting active links in a navigation menu or performing route-specific logic.
```
import Link from 'next/link';
import usePathname from '../hooks/usePathname'; // Adjust the path as needed
...
const ActiveLink = ({ href, children }) => {
  ...
  return (
    <Link href={href}>
      <a style={{ color: isActive ? 'red' : 'black' }}>
        {children}
      </a>
    </Link>
  );
